...
if __name__ == '__main__':
    ...
    if args.libri_dir is not None:
        train_folders = [x for x in glob.glob(os.path.join(args.libri_dir, 'testData', '*'))]
    ...
    # 遍历数据集并生成训练和测试样本
    for i in tqdm.tqdm(range(1000), desc='Generating train samples'):
        train_wrapper(i, hp, args, audio, train_spk)

    for i in tqdm.tqdm(range(0), desc='Generating test samples'):
