...
def train(args, pt_dir, chkpt_path, trainloader, testloader, writer, logger, hp, hp_str):
    ...
    try:
        ...
        while True:
            model.train()
            epsilon = 1e-16  # 小的epsilon值，防止数值不稳定
            for dvec_mels, target_mag, mixed_mag in trainloader:
                ...
                # 在输出和目标上加上epsilon，防止数值不稳定
                output_tensor = output_tensor + epsilon
                target_tensor = target_tensor + epsilon

                # 计算损失Power-law compression
                if 0:
                    magnitude_loss = criterion(
                        torch.abs(torch.pow(output_tensor, hp.audio.power)),
                        torch.abs(torch.pow(target_tensor, hp.audio.power)),
                    )
                    complex_loss = criterion(
                        torch.pow(output_tensor, hp.audio.power),
                        torch.pow(target_tensor, hp.audio.power),
                    )
                else:
                    output_tensor = torch.pow(output_tensor, hp.audio.power)
                    target_tensor = torch.pow(target_tensor, hp.audio.power)

                    magnitude_loss = criterion(torch.abs(target_tensor), torch.abs(output_tensor))
                    complex_loss = criterion(target_tensor, output_tensor)
